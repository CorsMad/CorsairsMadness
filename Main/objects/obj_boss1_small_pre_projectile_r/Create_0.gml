/// @description Insert description here
// You can write your code in this editor

image_angle = choose(0,90,180,270);
if instance_exists(obj_boss1_phase2)
{
	x = obj_boss1_phase2.x+21;
	y = obj_boss1_phase2.y-7;
}

if instance_exists(obj_boss1_phase3)
{
	x = obj_boss1_phase3.x+21;
	y = obj_boss1_phase3.y-7;
}
if instance_exists(obj_boss1e_phase2)
{
	x = obj_boss1e_phase2.x+21;
	y = obj_boss1e_phase2.y-7;
}

if instance_exists(obj_boss1e_phase3)
{
	x = obj_boss1e_phase3.x+21;
	y = obj_boss1e_phase3.y-7;
}
